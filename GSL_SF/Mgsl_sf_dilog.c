/* MARIADB UDF: WRAPPER FOR /usr/include/gsl/gsl_sf_dilog.h
-------------------------------------------------------------------
COMPILE:
    gcc -o Mgsl_sf_dilog.so -DHAVE_DLOPEN -Wall -shared -lstdc++  Mgsl_sf_dilog.c  -I/usr/include/mariadb -I/usr/include -I. -fPIC -lgsl
    -------------------------------------------------------------------
INSTALL:
    sudo cp Mgsl_sf_dilog.so /usr/lib/mysql/plugin
    sudo chmod 777 /usr/lib/mysql/plugin/Mgsl_sf_dilog.so
    sudo service mariadb restart
------------------------------------------------------------------- 
LOAD:
            DROP FUNCTION IF EXISTS M;
            CREATE function M RETURNS REAL SONAME 'Mgsl_sf_dilog.so';            
-------------------------------------------------------------------
*/
...
#include "gsl/gsl_sf_dilog.h"
    ...
    extern "C" {
            ...
            // SRC FILE     : http://git.savannah.gnu.org/cgit/gsl.git/tree/specfunc/gsl_sf_dilog.h
            // SRC FUNCTION : double  gsl_sf_dilog(const double x);
            ...
            // UDF functions for double  gsl_sf_dilog(const double x);
